...
import math
from sports.tennis.rankings import get_ranking, get_surface_strength, get_surface_elo, detect_tour

# ── ML модель (XGBoost) ───────────────────────────────────────────────────────
try:
    import sys, os as _os
    _ml_dir = _os.path.join(_os.path.dirname(__file__), "..", "..", "ml")
    if _ml_dir not in sys.path:
        sys.path.insert(0, _os.path.abspath(_ml_dir))
    from predictor_tennis import predict_tennis_winner as _ml_predict
    _ML_AVAILABLE = True
except Exception:
    _ML_AVAILABLE = False

_SURFACE_ML = {"hard": "Hard", "clay": "Clay", "grass": "Grass", "carpet": "Hard"}


def rank_to_elo(rank: int) -> float:
    """
    Конвертирует ATP/WTA рейтинг в Elo-подобный балл.
    Топ-1 ≈ 2350, топ-10 ≈ 2150, топ-50 ≈ 1870, #200 ≈ 1500.
    Уменьшен множитель 400→300 чтобы близкие ранки не давали слишком
    большого преимущества (#9 vs #19 теперь ~58%, а не 68%).
    """
    if rank <= 0:
        rank = 200
    return round(2500 - 300 * math.log10(max(1, rank)), 1)
...
def calculate_tennis_probs(
    ...
    no_vig_p2: float = 0.0,
    tournament_level: str = "A",   # G/M/A/D — для ML модели
    round_str: str = "R32",        # F/SF/QF/R16/R32 — для ML модели
    p1_age: float = 26.0,
    p2_age: float = 26.0,
) -> dict:
    """
    ...
    # Surface ELO (персистентный, обновляется после матчей; фолбэк — rank + surface_strength)
    p1_elo = get_surface_elo(player1, surface, tour)
    p2_elo = get_surface_elo(player2, surface, tour)
    ...
    if no_vig_p1 > 0.05 and no_vig_p2 > 0.05:
        ...
        # Но если модель и Pinnacle оба сильно расходятся с рынком — ограничиваем
        p1_win = p1_win * 0.30 + no_vig_p1 * 0.70
        p1_win = max(0.08, min(0.92, p1_win))
        ...
        if model_market_gap > 0.15:
            # Расхождение > 15% — рынок почти всегда прав, доверяем сильно
            p1_win = p1_win * 0.20 + bk_p1 * 0.80
        elif model_market_gap > 0.08:
            # Умеренное расхождение — рынок имеет преимущество
            p1_win = p1_win * 0.35 + bk_p1 * 0.65
        else:
            # Модель и рынок согласны — равный вес
    ...
    if _ML_AVAILABLE:
        try:
            # Форма: "WWLWW" → доля побед 0.0-1.0
            def _form_rate(f: str) -> float:
                if not f or f == "?????":
                    return 0.5
                wins = f.upper().count("W")
                return wins / max(len(f), 1)

            ml_surface = _SURFACE_ML.get(surface.lower(), "Hard")
            ml_result = _ml_predict(
                p1_name=player1,   p1_rank=p1_rank,
                p2_name=player2,   p2_rank=p2_rank,
                surface=ml_surface,
                level=tournament_level,
                round_str=round_str,
                p1_form=_form_rate(p1_form),
                p2_form=_form_rate(p2_form),
                p1_age=p1_age, p2_age=p2_age,
                h2h_adv=(h2h_p1_wins / h2h_total - 0.5) if h2h_total >= 3 else 0.0,
                h2h_total=h2h_total,
            )
            if ml_result and "prob" in ml_result:
                # ML говорит о фаворите по рейтингу — нужно привести к P1-перспективе
                ml_fav_is_p1 = (p1_rank <= p2_rank)
                ml_p1_win = ml_result["prob"] if ml_fav_is_p1 else (1.0 - ml_result["prob"])
                # Ансамбль: ELO+рынок 65% + ML 35%
                # Рынок (Pinnacle) — лучший предиктор в теннисе, XGBoost дополняет
                p1_win = p1_win * 0.65 + ml_p1_win * 0.35
                p1_win = max(0.05, min(0.95, p1_win))
                ml_bet = ml_result.get("bet", False)
        except Exception:
            pass
...
def compute_tennis_chimera_score(
    ...
    for player, prob, odds, rank_fav, rank_opp, form, h2h_wins, outcome_key in [
        ...
        # Ограничение коэффициентов: > 2.5 в теннисе стабильно проигрывают (данные: 0W/4L)
        if not odds or odds < 1.55 or odds > 2.5 or prob <= 0:
            continue
